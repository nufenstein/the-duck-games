...
class GameOneScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.platforms.getChildren().forEach(platform => {
        let gem = this.gems.create(platform.x, platform.y - 40, 'gem').setScale(0.18);
        gem.refreshBody();
      });
      ...
      // Jump Mechanic:
      // On key down, start jump immediately with a small jump.
      // The jump height is adjusted on key up based on the press duration,
      // but capped so that the maximum jump height is 25% of the screen height.
      ...
      // On key up, compute the press duration (up to 1.5 sec).
      // If the press lasted at least 1 second, adjust the jump to achieve 25% of screen height;
      // otherwise, keep it at 10% of screen height.
      this.input.keyboard.on('keyup-UP', function () {
        if (this.isJumping) {
          ...
          // If held for at least 1 second, target the super jump;
          // otherwise, target the small jump.
          let desiredHeight = pressDuration >= 1000 ? h_super : h_small;
          let v_desired = Math.sqrt(2 * g * desiredHeight);
          // Apply the new velocity (ensure upward velocity is negative).
      ...
      // Collisions: allow duck-platform collision only when falling.
      this.physics.add.collider(this.duck, this.platforms, null, (duck, platform) => {
        return duck.body.velocity.y >= 0;
      }, this);
